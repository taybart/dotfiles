...
app.breakCallback = function()
  if not currentBreak then
    return
  end
  currentBreak.minutesLeft = currentBreak.minutesLeft - 1
  if currentBreak.minutesLeft <= 0 then
    app.completeBreak(currentPomo)
  end
  app.updateUI()
end
...
app.completePomo = function(pomo)
  ...
  log.writeItem(pomo)
  currentPomo = nil
  currentBreak = { minutesLeft = BREAK_LENGTH }

  if timer then
    timer:stop()
  end

  timer = hs.timer.doEvery(INTERVAL_SECONDS, app.breakCallback)
end

app.completeBreak = function(pomo)
  currentPomo = nil
  currentBreak = nil
...
app.getMenubarTitle = function(pomo)
  ...
  elseif currentBreak then
    title = title .. ('0:' .. string.format('%02d', currentBreak.minutesLeft))
  end
  return title
end
